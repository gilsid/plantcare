...
class PlantProvider extends ChangeNotifier {
  ...
  Future<int> addPlant({
    ...
    String? location,
    required List<
      ({CareType type, int intervalValue, IntervalUnit intervalUnit})
    >
    careTasks,
    String? photoPath,
    String? notes,
    ...
    for (final ct in careTasks) {
      ...
      try {
        debugPrint(
          '[PlantProvider] addPlant: scheduling notification for ${ct.type}',
        );
        ...
        debugPrint(
          '[PlantProvider] addPlant: ❌ notification FAILED for ${ct.type}: $e',
        );
        debugPrintStack(stackTrace: stackTrace);
    ...
    notifyListeners();
    debugPrint(
      '[PlantProvider] addPlant: done ($notificationFailures notification failures)',
    );
  ...
  Future<void> updatePlant({
    ...
    if (index != -1) {
      ...
      if (photoPath != null &&
        ...
        final isReferencedByDiary = plant.growthDiary.any(
          (entry) => entry.photoPath == plant.photoPath,
        );
  ...
  Future<void> deletePlant(String id) async {
    ...
    if (index != -1) {
      ...
      for (final task in tasks) {
        await _notificationService.cancelCareReminder(id, task.careType);
      }
  ...
  Future<bool> completeCareTask(
    ...
    final plant = _plants.firstWhere(
      (p) => p.id == plantId,
      orElse: () =>
          throw StateError('[completeCareTask] Plant not found: $plantId'),
    );
  ...
  Future<void> updateCareTask({
    ...
    final task = _careTasks.firstWhere(
      (t) => t.id == taskId,
      orElse: () =>
          throw StateError('[updateCareTask] Task not found: $taskId'),
    );
  ...
  Future<void> removeCareTask(String taskId) async {
    final task = _careTasks.firstWhere(
      (t) => t.id == taskId,
      orElse: () =>
          throw StateError('[removeCareTask] Task not found: $taskId'),
    );
  ...
  ({IconData icon, Color color, String label})? getHealthTrend(String plantId) {
    final histories =
        _careHistories
            .where(
              (h) => h.plantId == plantId && h.careType == CareType.watering,
            )
            .toList()
    ...
    if (diff.abs() < 0.05) {
      return (icon: Icons.trending_flat, color: Colors.grey, label: 'Stabil');
    }
    if (diff > 0) {
      return (
  ...
  double _calculateHealthScore(Plant plant) {
    final histories =
        _careHistories
            .where(
              (h) => h.plantId == plant.id && h.careType == CareType.watering,
            )
            .toList()
          ..sort((a, b) => b.completedAt.compareTo(a.completedAt));
    ...
    if (recentHistories.length >= 5) {
      onTimeFlags = recentHistories
          .take(5)
          .map((h) => h.wasOnTime ?? true)
          .toList();
