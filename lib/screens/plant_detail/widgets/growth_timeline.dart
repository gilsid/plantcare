...
behavior: SnackBarBehavior.floating,
  ...
  void _showCompareDialog() {
    final diary = widget.plant.growthDiary;
    if (diary.length < 2) return;
    final first = diary.last;
    final last = diary.first;
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('Perbandingan Pertumbuhan', textAlign: TextAlign.center),
        content: Row(
          children: [
            Expanded(child: _buildCompareCard(first, 'Awal')),
            const Padding(
              padding: EdgeInsets.symmetric(horizontal: 8),
              child: Icon(Icons.arrow_forward, color: AppColors.primaryLight),
            ),
            Expanded(child: _buildCompareCard(last, 'Terbaru')),
          ],
        ),
        actions: [
          TextButton(
  ...
  Widget _buildCompareCard(GrowthEntry entry, String label) {
    final textTheme = Theme.of(context).textTheme;
    ...
    return Column(
      ...
      children: [
        ...
        Text(formattedDate, style: textTheme.bodySmall?.copyWith(fontSize: 10)),
      ],
    );
  }
...
label: const Text('Tambah Foto Pertama'),
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        itemCount: widget.plant.growthDiary.length +
            1 +
            (widget.plant.growthDiary.length >= 2 ? 1 : 0),
        ...
        itemBuilder: (context, index) {
          ...
          if (widget.plant.growthDiary.length >= 2 && index == 1) {
            return Padding(
              padding: const EdgeInsets.only(bottom: 16.0),
              child: OutlinedButton.icon(
                onPressed: _showCompareDialog,
                icon: const Icon(Icons.compare_arrows, size: 18),
                label: const Text('Bandingkan Awal & Terbaru'),
                style: OutlinedButton.styleFrom(
                  minimumSize: const Size(double.infinity, 50),
                  side: BorderSide(
          ...
          final entryIndex = widget.plant.growthDiary.length >= 2
              ? index - 2
              : index - 1;
          final entry = widget.plant.growthDiary[entryIndex];
...
width: 2.5,
          ...
          if (entryIndex != widget.plant.growthDiary.length - 1)
            Container(
width: 2.5,
height: 200,
color: isDark
